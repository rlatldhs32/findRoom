...
public class ResponseUtil {
    ...
    public static <T> ResponseEntity<ResponseDTO<T>> fail(T response) { //요청에 맞춰 수정
        return ResponseEntity.ok(ResponseDTO.fail(response));
    }

    public static <T> ResponseEntity<ResponseDTO<T>> error(HttpHeaders headers, HttpStatus status, String message) {
    ...
    public static class ResponseDTO<T> implements Serializable {
        ...
        public static <T> ResponseDTO<T> fail(T response) {
            return new ResponseDTO<>(false, response, null);
        }
